...
// service de recherche date
    ...
    /********si salle existe ne sera pas effectuer a un autre spectacle*********/
    /*
    public boolean salleTaken(int idS) throws SQLException {

        String req = "SELECT `idSalle` FROM `spectacle` WHERE `idSalle` = '" + idS + "'";
        SingletonConnection ds1 = SingletonConnection.getInstance();
        Connection conn = ds1.getCnx();
        PreparedStatement pst = conn.prepareStatement(req);
        rs = pst.executeQuery(req);
